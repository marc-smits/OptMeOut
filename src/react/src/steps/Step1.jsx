        ...
        <div className="row">
          <div className="col col-10">
            <div className="optOutToggle">
              <div
                className={"checkBox " + optOut.selected[data.index]}
                onClick={() => {
                  toggleOptOut(data.index);
                }}
              >
            ...
            {Object.keys(data.data.p).map((innerAttr, index) => {
              return <p>{data.data.p[innerAttr]}</p>;
            })}

            <div
              className={
                readMore["selected"][data.index] ? "readMore show" : "readMore"
              }
            >
              {data.data.readmore}
            </div>
            <div className="flex-end">
              <ButtonMore
                readMore={readMore["selected"][data.index]}
                toggleReadMore={() => {
                  toggleReadMore(data.index);
                }}
              />
            </div>
          </div>
      ...
      {/* Tell me more */}
      {
        <div className={tellMeMore ? "readMore show" : "readMore"}>
          ...
          {Object.keys(optOuts).map((innerAttr, index) => {
            return <Readmore data={optOuts[innerAttr]} index={index} />;
          })}
        </div>
      }
      ...
      <div className="row">
        <div className="col flex-center">
          ...
          <div
            ...
            onClick={
              !disableNextStep
                ? (e) => props.emitChangeSection("step2", e)
                : void 0
            }
          >
            [[button.OptMeOut]]
